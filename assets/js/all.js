(function($){
	...
	for( l; l > 0; l-- ){ $('.swipe-section:eq('+(l-1)+')').css('z-index',x); x++; }
    s.css({
    ...
    function swipeFront() {
    	if(c < (b-1)){
        	$('.swipe-section:eq('+c+')').stop().transition({
        		...
        		if(c == 0){ $('.swipe-section:eq('+c+')').css('z-index',((c+1)*10)); }
        		c++;
        		a--;
        	});
        	if(c != 0){ $('.swipe-section:eq('+c+')').css('z-index',((c+1)*10)); }
    ...
    s.children('div.front').swipe({swipeUp:swipeFront}).on('click',swipeFront);
    s.children('div.back').swipe({swipeDown:swipeBack}).on('click',swipeBack);
    $('i.swipe-button.up').on('click',swipeFront);
    $('i.swipe-button.down').on('click',swipeBack);
...
function init_google_map(){
    (function($){
        ...
        $(".see-map").click(function(){
            ...
            var me = $(this);
            console.debug(me.data('rel'));
            
            gmMapDiv = $(me.data('rel'));
            
            me.toggleClass("js-active");
            ...
            gmMapDiv.slideToggle(400, function(){
                ...
                if (me.hasClass("js-active")) {
                    
                    console.debug('append', gmMapDiv);
...
function init_map(){
    (function($){
        if (gmMapDiv.length) {
            ...
            console.debug(gmCenterAddress);
            console.debug(gmMarkerAddress);
